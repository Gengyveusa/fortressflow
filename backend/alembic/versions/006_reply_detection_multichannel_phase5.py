...
def upgrade() -> None:
    # ── reply_logs ────────────────────────────────────────────────────────
    # Tracks every inbound reply across all channels with sentiment +
    # full AI analysis results. Matched to enrollments via thread_id or
    # sender email. Body is stored as a 500-char snippet to keep table lean.
    op.create_table(
        "reply_logs",
        sa.Column(
            "id",
    ...
    # Raw event log for all inbound webhook events (Twilio, SES, Parsio, IMAP).
    # Processed flag allows idempotent re-processing of unhandled events.
    op.create_table(
        "reply_webhook_events",
        sa.Column(
            "id",
    ...
    # ── linkedin_queue ────────────────────────────────────────────────────
    # Persistent LinkedIn outreach queue. Items are created by the sequence
    # engine / hole-filler and executed by the linkedin queue worker task.
    # Stores the full LinkedInPayload JSON so items survive service restarts.
    op.create_table(
        "linkedin_queue",
        sa.Column(
            "id",
    ...
    # ── channel_metrics ───────────────────────────────────────────────────
    # Daily aggregation table for per-channel delivery metrics.
    # Populated by the aggregate_channel_metrics_task Celery beat job (hourly).
    # Unique constraint on (channel, date) enforces one row per channel per day.
    op.create_table(
        "channel_metrics",
        sa.Column(
            "id",
    ...
    # reply_logs indexes
    op.create_index(
        "ix_reply_logs_received_at",
        "reply_logs",
        ["received_at"],
    )
    op.create_index(
        "ix_reply_logs_enrollment_id",
        "reply_logs",
        ["enrollment_id"],
    )
    op.create_index(
        "ix_reply_logs_sentiment",
        "reply_logs",
        ["sentiment"],
    )

    # reply_webhook_events indexes
    op.create_index(
    ...
    # linkedin_queue indexes
    op.create_index(
        "ix_linkedin_queue_status",
        "linkedin_queue",
        ["status"],
    )
    op.create_index(
        "ix_linkedin_queue_scheduled_at",
        "linkedin_queue",
        ["scheduled_at"],
    )

    # channel_metrics composite index (mirrors the unique constraint for fast lookups)
    op.create_index(
    ...
    # sequence_enrollments.last_touch_at — enables efficient "due for next step"
    # queries in the sequence engine (previously would full-scan enrollments).
    op.create_index(
    ...
    # touch_logs(channel, created_at) — enables per-channel daily count queries
    # used by the ChannelOrchestrator.check_global_limits() and metrics tasks.
    op.create_index(
