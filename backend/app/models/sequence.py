...
class SequenceStep(Base):
    ...
    step_type: Mapped[StepType] = mapped_column(
        Enum(StepType, name="step_type", create_type=False), nullable=False
    )
...
class SequenceEnrollment(Base):
    ...
    ab_variant_assignments: Mapped[dict | None] = mapped_column(
        JSONB, nullable=True
    )  # {step_position: "A" or "B"}

    # Phase 4: Hole-filler tracking
    hole_filler_triggered: Mapped[bool] = mapped_column(
        Boolean, default=False, nullable=False
    )
    escalation_channel: Mapped[str | None] = mapped_column(
