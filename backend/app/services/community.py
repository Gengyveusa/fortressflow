...
class CommunityService:
        ...
        self._onboarding_sequence = [
            {
                ...
                "content": "You're one of the select few...",
            },
            {
                "day": 1,
                "step": OnboardingStep.PROFILE_SETUP,
                "subject": "Complete Your Profile",
                "content": "Stand out in the community...",
            },
            {
                "day": 2,
                "step": OnboardingStep.FIRST_CONNECTION,
                "subject": "Make Your First Connection",
                "content": "The power of this community is its people...",
            },
            {
                "day": 3,
                "step": OnboardingStep.FIRST_CONTENT,
                "subject": "Exclusive Research Awaits",
                "content": "Access research only available to members...",
            },
            {
                "day": 5,
                "step": OnboardingStep.FIRST_EVENT,
                "subject": "Your First Exclusive Event",
                "content": "Join experts for a live session...",
            },
            {
                "day": 6,
                "step": OnboardingStep.COMMUNITY_INTRO,
                "subject": "Introduce Yourself",
                "content": "Share your expertise with fellow members...",
            },
            {
                "day": 7,
                "step": OnboardingStep.FEATURE_TOUR,
                "subject": "Unlock All Features",
                "content": "Discover early access features...",
            },
        ]
    ...
    def join_waitlist(
        self, email: str, company: str, role: str, referral_source: Optional[str] = None
    ) -> WaitlistEntry:
    ...
    def redeem_invite(self, code: str, user_id: str, email: str, name: str, company: str) -> Optional[CommunityMember]:
        ...
        member = CommunityMember(
            user_id=user_id, email=email, name=name, company=company, tier=invite.tier, badges=["early_adopter"]
        )
    ...
    def get_onboarding_sequence(self, member_id: str) -> list[dict]:
        ...
        return [{**step, "completed": step["step"].value in completed} for step in self._onboarding_sequence]
    ...
    def get_community_stats(self) -> dict:
        return {
            ...
            "active_events": len(
                [e for e in self._events.values() if e.scheduled_at and e.scheduled_at > datetime.now(UTC)]
            ),
            "tier_distribution": {
                t.value: sum(1 for m in self._members.values() if m.tier == t)
                for t in MembershipTier
                if t != MembershipTier.WAITLIST
            },
    ...
    def get_fomo_metrics(self) -> dict:
        return {
            ...
            "next_event": next(
                (
                    {"title": e.title, "spots_left": e.max_attendees - len(e.registered)}
                    for e in self._events.values()
                    if e.scheduled_at and e.scheduled_at > datetime.now(UTC)
                ),
                None,
            ),
