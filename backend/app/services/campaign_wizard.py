...
class CampaignWizard:
    ...
    async def start_campaign(
        ...
        try:
            ...
            for i, step in enumerate(sequence_preview.get("steps", [])[:seq_length]):
                ...
                day = sum(s.get("delay_hours", 0) for s in sequence_preview.get("steps", [])[:i]) / 24
            ...
            channel_summary = " + ".join(f"{v} {k}{'s' if v > 1 else ''}" for k, v in channel_counts.items())
    ...
    async def _check_compliance(self, leads: list) -> list:
        ...
        async with AsyncSessionLocal() as db:
            # Check DNC list
            dnc_result = await db.execute(select(DNCBlock.identifier).where(DNCBlock.identifier.in_(lead_emails)))
